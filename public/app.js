...
Highcharts.chart('StrikeRate', {
  ...
  series: [{
      name: 'Population', // If we don't want to display the dataLabel onto the graph then ommit this line and dataLabel{}.
      data: seriesData,
      dataLabels: {
          enabled: true,
          rotation: -90,
...
//Bowlers Economy IPL(6th Problem using express sever)

function myFunction(){
  var input = document.getElementById("work").value;
  
  fetch("/economy?year=" + input)
      .then(res => res.json())
      .then(visualizeData6);
  
  
  function visualizeData6(data) {
    visualizeBowlersEconomyIPL(data);
    return;
  }
function visualizeBowlersEconomyIPL(BowlersEconomyIPL) {
  const seriesData = [];
  for (let year in BowlersEconomyIPL) {
    seriesData.push([year,BowlersEconomyIPL[year]]);
  }

  Highcharts.chart("Ipl-bowlers-economy", {
    chart: {
      type: "column"
    },
    title: {
      text: "Top 10 Economy of Bowlers"
    },
    subtitle: {
      text:
        'Source: <a href="https://www.kaggle.com/nowke9/ipldata/data">IPL Dataset</a>'
